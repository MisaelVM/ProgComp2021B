...
std::vector<std::pair<int, int>> suffix_array(const std::string& str, const int shift) {
	int n = str.size();
...
void shift_alphabet(std::string& T, int n) {
	for (int i = 0; i < (int)T.size(); ++i)
		T[i] += n;
}

void unshift_alphabet(std::string& T, int n) {
	for (int i = 0; i < (int)T.size(); ++i) {
		T[i] -= n;
	}
}

std::set<std::string> longest_common_substring(std::vector<std::string>& strs, int n, int k) {
	char sentinel = 1;
	std::string T = "";
	for (auto& str : strs) {
		shift_alphabet(str, n);
		T += str;
		T += sentinel++;
	}

	std::vector<std::pair<int, int>> SA = suffix_array(T, n);
	std::vector<std::pair<int, int>> LCP = lcp_array(T, n, SA);

	unshift_alphabet(T, n);
...
int main()
{
	...
	n = 3;
	k = 2;
	print_set(longest_common_substring(S, n, k));

	// Caso de prueba 2
	// Entrada:
	// S = ["AABC","BCDC","BCDE","CDED"]
	// n = 4
	// k = 2
	// Salida:
	// ["BCD","CDE"]
